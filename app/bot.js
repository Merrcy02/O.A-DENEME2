...
client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "watching" }, status: "dnd" });
});
});
...
client.on("message", async msg => {
 const i = await db.fetch(`${msg.guild.id}.kufur`)
    if (i) {
        const kufur = ["oc","oç","laxenthia","oyunreklam","oyunalanı","0c", "ananızıskm", "orusbucocu", "0ruspucocu", "p1c", "0rusbucocu","allahını sikim", "kitabını sikim", "4nan4skm", "0evladı", "4nanı","4nan4skm", "s2m", "ananı sikiyim", "ananıskm", "piç", "amk", "amsk", "sikim", "sikiyim", "orospu çocuğu", "piç kurusu", "kahpe", "orospu", "mal", "sik", "yarrak", "am", "amcık", "amık", "yarram", "sikimi ye", "mk", "mq", "aq", "ak", "amq",];
        if (kufur.some(word => msg.content.includes(word))) {
          try {
            if (!msg.member.permissions.has("BAN_MEMBERS")) {
                  msg.delete();
                          
                      return msg.reply('Heey! Küfür veya Yasaklı Kelime Kullandınız.').then(nordx => nordx.delete({timeout: 5000}))
            }              
          } catch(err) {
            console.log(err);
          }
...
client.on("messageUpdate", async msg => {
 const i = db.fetch(`${msg.guild.id}.kufur`)
    if (i) {
        const kufur = ["oc","oç","laxenthia","oyunalanı","oyunreklam","0c", "ananızıskm", "orusbucocu", "0ruspucocu", "p1c", "0rusbucocu","allahını sikim", "kitabını sikim", "4nan4skm", "0evladı", "4nanı","4nan4skm", "s2m", "ananı sikiyim", "ananıskm", "piç", "amk", "amsk", "sikim", "sikiyim", "orospu çocuğu", "piç kurusu", "kahpe", "orospu", "mal", "sik", "yarrak", "am", "amcık", "amık", "yarram", "sikimi ye", "mk", "mq", "aq", "ak", "amq",];
        if (kufur.some(word => msg.content.includes(word))) {
          try {
            if (!msg.member.permissions.has("BAN_MEMBERS")) {
                  msg.delete();
                          
                      return msg.reply('Yakaladım Seni! Küfür veya Yasaklı Kelime Kullandınız.').then(nordx => nordx.delete({timeout: 5000}))
            }              
          } catch(err) {
            console.log(err);
          }
